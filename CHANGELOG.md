...
* The picker now writes its selection to the Livewire component that owns the form (`$wire.$parent`) instead of looking up a component named `app.filament.resources.*`. Filament only renders the modal of the topmost mounted action, so when the picker is opened from a nested action modal (e.g. a block editor) the field listening for `picked-resource` was not in the DOM and the selection was lost.
